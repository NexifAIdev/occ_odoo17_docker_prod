...
if ! sudo -u postgres psql -tAc "SELECT 1 FROM pg_roles WHERE rolname='$OE_MGR';" | grep -q 1; then
    sudo su - postgres -c "createuser -s $OE_MGR" 2>/dev/null || true
else
    printf "Database user '%s' already exists, skipping creation.\n" "$OE_MGR"
fi

sudo -u postgres psql -c "ALTER USER $OE_MGR WITH PASSWORD '$OE_MGR';" 2>/dev/null || true
sudo -u postgres psql -c "ALTER USER $OE_MGR WITH SUPERUSER;" 2>/dev/null || true
